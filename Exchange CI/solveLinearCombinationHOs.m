...
%     end
    ...
    SLOHO = zeros(sparams.nSingleOrbitals);
    for ii = 1:(sparams.nSingleOrbitals)
        currWFLeft = sparams.LOHOs(ii).wavefunctionMG;
        for jj = 1:(sparams.nSingleOrbitals)
            currWFRight = sparams.LOHOs(jj).wavefunctionNO;
            HLOHO(ii,jj) = getInnerProduct2D(currWFLeft,...
                convertNOtoMG(full2DLap*currWFRight,sparams.ngridx,sparams.ngridy),X,Y);
            SLOHO(ii,jj) = getInnerProduct2D(currWFLeft,sparams.LOHOs(jj).wavefunctionMG,X,Y);
        end
    end
    ...
    for ii = 1:sparams.nSingleOrbitals
        ...
        for jj = 1:sparams.nSingleOrbitals
            tempwf = tempwf + sparams.acoeffs(ii,jj)*sparams.LOHOs(jj).wavefunctionMG;
        end
