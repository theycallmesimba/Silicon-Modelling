function CME = calculateOriginCMEGPU(na, ma, nb, mb, ng, mg, nd, md)
    ...
    for p1 = 0:min(na,nd) 
        coef1 = fact(p1)*nck(na,p1)*nck(nd,p1);
        % Continue building a and p
        a1 = a0 - 2*p1;
        pInd1 = pInd0 - 2*p1;
        
        for p2 = 0:min(ma,md)
            coef2 = coef1*fact(p2)*nck(ma,p2)*nck(md,p2);
            % Continue building p
            pInd2 = pInd1 - 2*p2;
            
            for p3 = 0:min(nb,ng)
                coef3 = coef2*fact(p3)*nck(nb,p3)*nck(ng,p3);
                % Finish building a and continue building and p
                a = a1 - 2*p3;
                pInd3 = pInd2 - 2*p3;
        
                for p4 = 0:min(mb,mg)                    
                    coef4 = coef3*fact(p4)*nck(mb,p4)*nck(mg,p4);
                    % Finish building p
                    p = (pInd3 - 2*p4)/2;
    ...
    CME = CME/sqrt(fact(na)*fact(ma)*fact(nb)*fact(mb)*...
        fact(ng)*fact(mg)*fact(nd)*fact(md));
end
